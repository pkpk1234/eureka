 ...
 * container, which is essential to verifying content encoding/decoding with different format types
 * (JSON vs XML, compressed vs uncompressed).
 *
 * @author Tomasz Bak
 */
public class EurekaClientServerRestIntegrationTest {
  ...
  public static void setUp() throws Exception {
    ...
    httpClientFactory =
        JerseyEurekaHttpClientFactory.newBuilder()
    ...
    jerseyReplicationClient =
        JerseyReplicationClient.createReplicationClient(
            eurekaServerConfig, serverCodecs, eurekaServiceUrl);
  }
  ...
  public void testHeartbeat() throws Exception {
    ...
    EurekaHttpResponse<InstanceInfo> heartBeatResponse =
        jerseyReplicationClient.sendHeartBeat(
  ...
  public void testStatusOverrideUpdateAndDelete() throws Exception {
    ...
    EurekaHttpResponse<Void> overrideUpdateResponse =
        jerseyEurekaClient.statusUpdate(
            instanceInfo.getAppName(), instanceInfo.getId(), InstanceStatus.DOWN, instanceInfo);
    ...
    EurekaHttpResponse<Void> deleteOverrideResponse =
        jerseyEurekaClient.deleteStatusOverride(
  ...
  private static File findWar() {
    ...
    if (dir == null) {
      throw new IllegalStateException(
          "No directory found at any in any pre-configured location: "
              + Arrays.toString(EUREKA1_WAR_DIRS));
    }
    ...
    if (warFiles.length > 1) {
      throw new IllegalStateException(
          "Multiple war files found in directory " + dir + ": " + Arrays.toString(warFiles));
    }
    return warFiles[0];
  }
